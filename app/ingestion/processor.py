...
def save_processed_local(data: dict, source_type: str, filename: str) -> str:
...
save_processed_locally = save_processed_local
...
def process_file(file_path: str, filename: str, source_type: str):
    ...
    with logfire.span("Processing File", file=filename, source=source_type):
        try:
            ...
            local_path = save_processed_local(processed_data, source_type, filename)
...
def run_universal_ingestion(
    ...
    """
    Scan base_dir, map sub-folder to source type, and ingest all documents.
    Pass --wipe to drop and recreate the Qdrant collection before ingestion.
    """
    with logfire.span("Universal Ingestion Started", base_directory=base_dir):
        # Drop and recreate collection if wipe requested
        if wipe and qdrant_client.collection_exists(settings.QDRANT_COLLECTION):
            logfire.info(f"Wiping collection '{settings.QDRANT_COLLECTION}' before ingestion.")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m app.ingestion.processor <data_path> [source_type] [--wipe]")
        print("Example: python -m app.ingestion.processor DATA/true_data true --wipe")
        sys.exit(1)
    ...
    # If ingest clean data command is run with 'true', default wipe if --wipe is specified
    run_universal_ingestion(
        base_dir=target_path,
        explicit_source_type=parsed_source_type,
        wipe=should_wipe,
    )